...
def run_model(
    ...
    for batch_start in range(0, len(starts), chunk_batch_size):
        ...
        for start in batch_starts:
            ...
            # the first chunk's window begins at the file start and the last
            # chunk's ends at the file end, extending further left instead —
            # the model is trained on audio that is cut abruptly, and
            # silence belongs nowhere inside a real file.
            window_start = start - chunk_pad_samples if start else 0
            if chunk_pad_samples and end == total_samples:
                window_start = max(0, total_samples - padded_chunk_samples)
            chunk = audio[..., window_start : window_start + padded_chunk_samples]
        ...
        for offset, (start, valid_samples) in enumerate(
            ...
            core = chunk_pad_samples if start else 0
            if chunk_pad_samples and end == total_samples:
                core = start - max(0, total_samples - padded_chunk_samples)
            chunk_output = batch_output[
                offset : offset + 1, ..., core : core + valid_samples
            ]
