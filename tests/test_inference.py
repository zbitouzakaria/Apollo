...
class DegradedEdgesModel(IdentityModel):
    """Identity, except both edges of every window come back wrong.
...
class InferenceTests(unittest.TestCase):
    ...
    def test_chunk_padding_discards_degraded_window_edges(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            input_path = root / "input.wav"
            output_path = root / "output.wav"
            ...
            # discards exactly those samples. Padding is real neighboring
            # audio, so the first chunk starts at the file start and its
            # degraded head is kept — there is nothing before the file to
            # pad with, and the real model is trained on audio that begins
            # abruptly. The check therefore starts after the file's own
            # first samples; the final chunk's degraded end lands in the
            # silence that fills the fixed chunk size past the file end,
            # and is cropped.
            ...
            # Every degraded window edge falls in discarded padding, except
            # at the file's own edges: the first chunk's window begins at
            # the file start and the last chunk's ends at the file end, so
            # the file's outermost samples are the model's window edges —
            # exactly as they are in an unchunked pass — and the check
            # excludes them.
